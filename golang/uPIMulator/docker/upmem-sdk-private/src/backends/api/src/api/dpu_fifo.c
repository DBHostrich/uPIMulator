...
init_fifo(struct dpu_set_t dpu_set, struct dpu_fifo_link_t *link, char *fifo_name, bool is_input)
{
    ...
    DPU_RANK_FOREACH (dpu_set, rank) {
        ...
        // Default time_for_retry reduced from 10 us to 1 us (2026-05-26 PIMulator
        // optimization). The retry path in dpu_copy_to_wram_fifo now routes any
        // sub-50-us wait through sched_yield(), which avoids the usleep(10)
        // scheduler-quantum oversleep that drove the V1A 1-rank pathology
        // (per-DPU retry loop multiplied each retry by ~50-100us actual wait
        // due to usleep granularity). Users who want the legacy 10us pacing
        // can restore it via dpu_fifo_set_time_for_push_retries().
        link->rank_fifos[each_rank].time_for_retry = 1;
        link->rank_fifos[each_rank].min_free_budget = 0;
