...
dpu_thread_job_create(struct dpu_rank_t **ranks, uint32_t nr_ranks)
{
    ...
    for (uint32_t each_rank = 0; each_rank < nr_ranks; ++each_rank) {
        ...
        for (uint32_t each_thread = 0; each_thread < rank->api.nr_threads; each_thread++) {
            ...
            /* PIMulator Patch D was prototyped here (NUMA-aware worker
             * thread pinning via pthread_setaffinity_np to the rank's
             * numa_node CPUs). Measured on upmemcloud5 2026-05-26: severe
             * regression at sub-rank populations (nr_dpus=8 median 27us ->
             * 106us with 244ms max; nr_dpus=64 median 93us -> 48ms),
             * no reliable improvement at multi-rank (within noise).
             * Mechanism: NUMA pinning restricts the broadcast_possible
             * retry-loop sched_yield() target set to a narrow CPU subset;
             * when no other thread on that subset can make progress, the
             * yield becomes effective busy-wait. Patch D and our earlier
             * usleep->sched_yield patch interact badly at sub-rank.
             * Reverted; raw data in
             * tools/calibration/data/sdk_patches_2026-05-2x/patchd_rejected/. */
        }
    }
